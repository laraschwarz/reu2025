...
def run(
    ...
    we_path_diff = np.diff(np.array(we_path), axis=0) # compute differences between consecutive points (accounts for parallel paths)
    ...
    ns_path_diff = np.diff(np.array(ns_path), axis=0) # compute differences between consecutive points (accounts for parallel paths)
